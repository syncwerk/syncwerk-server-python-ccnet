from pyrpcsyncwerk import RpcsyncwerkClient, rpcsyncwerk_func, RpcsyncwerkError
...
class RpcClientBase(RpcsyncwerkClient):
    ...
    def _start_service(self, client):
        ...
        if rsp.code != "200":
            raise RpcsyncwerkError("Error received: %s %s (In _start_service)" % (rsp.code, rsp.code_msg))
        return req_id
        ...
        if rsp.code == SC_SERVER_RET:
            ...
            while True:
                ...
                if rsp.code == SC_SERVER_MORE:
                    ...
                    raise RpcsyncwerkError("Error received: %s %s (In Read More)" % (rsp.code, rsp.code_msg))

            return buf
        elif rsp.code == SC_PROC_DEAD:
            raise DeadProcError()
        else:
            raise RpcsyncwerkError("Error received: %s %s" % (rsp.code, rsp.code_msg))
        ...
        while True:
            try:
                ...
                if self.req_pool:
                    ...
                    self.pool.return_client(client)
                    return ret
            except (NetworkError, RpcsyncwerkError):
...
class CcnetThreadedRpcClient(RpcClientBase):
    ...
    @rpcsyncwerk_func("int", ["string", "string", "int", "int"])
    ...
    @rpcsyncwerk_func("object", ["int"])
    def get_emailuser_by_id(self, user_id):
        pass

    @rpcsyncwerk_func("objlist", ["string", "int", "int", "string"])
    ...
    @rpcsyncwerk_func("int", ["string", "int", "string", "int", "int"])
    ...
    @rpcsyncwerk_func("int", ["int", "string", "string", "int"])
    ...
    @rpcsyncwerk_func("objlist", ["int", "int", "string"])
    ...
    @rpcsyncwerk_func("int", ["int", "string", "int"])
